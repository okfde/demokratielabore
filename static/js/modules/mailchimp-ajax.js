...
var hasError = function (field) {
    ...
    if (validity.typeMismatch) {
        ...
        // Email
        if (field.type === 'email') return 'Bitte überprüfe deine E-Mail Adresse.';

        // URL
        if (field.type === 'url') return 'Bitte gib deine E-Mail Adresse ein.';
...
var showError = function (field, error) {
    ...
    $('.error-message').not('.mc-status').remove();
    ...
    if (!message) {
        ...
        $('.error-message').insertAfter($('.validate'));
...
window.displayMailChimpStatus = function (data) {
    ...
    if (data.result === 'error') {
        window.mcStatus.classList.remove('hidden');
...
var submitMailChimpForm = function (form) {
    ...
    mcStatus = $('.newsletter-wrap')[0].querySelector('.mc-status');
    if(mcStatus){
        mcStatus.classList.remove('hidden');
        mcStatus.classList.remove('error-message');
        mcStatus.innerHTML = 'subscribing...';
    }
